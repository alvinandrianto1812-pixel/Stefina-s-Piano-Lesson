...
export default function AdminPayments({ onStatsUpdate }) {
  ...
  const verifyPayment = async (paymentId) => {
    ...
    const { error } = await supabase
      .from("payments").update({ status: "verified" }).eq("id", paymentId);
    if (error) toast.error("Gagal verifikasi: " + error.message);
    else { await fetchAll(); toast.success("Pembayaran terverifikasi."); }
  };

  const rejectPayment = async (paymentId) => {
    if (!window.confirm("Tolak pembayaran ini?")) return;
    const { error } = await supabase
      .from("payments").update({ status: "rejected" }).eq("id", paymentId);
    if (error) toast.error("Gagal menolak: " + error.message);
    else { await fetchAll(); toast.success("Pembayaran ditolak."); }
  };
  ...
  const filteredPayments = useMemo(() => {
    return payments.filter((p) => {
      ...
      if (searchQuery) {
        ...
        const parentName = [q?.father_name, q?.mother_name].filter(Boolean).join(" & ").toLowerCase();
        const studentName = (q?.student_full_name || "").toLowerCase();
        const instrument = (q?.instrument || "").toLowerCase();
        const query = searchQuery.toLowerCase();
        if (!parentName.includes(query) && !studentName.includes(query) && !instrument.includes(query))
          return false;
      }
      return true;
    });
  ...
  const exportToCSV = () => {
    ...
    const headers = ["Tanggal", "Orang Tua", "Anak", "Instrumen", "Jadwal", "Nominal", "Status"];
    const rows = filteredPayments.map((p) => {
      let q = p.questionnaire_id
        ? questionnaireById.get(p.questionnaire_id)
        : pickQAsOf(p.user_id, p.created_at);
      const parentName = [q?.father_name, q?.mother_name].filter(Boolean).join(" & ") || "-";
      const studentName = q?.student_full_name || "-";
      const instrument = q?.instrument || "-";
      const schedule = q ? fmtPreferredSchedule(q.preferred_day, q.preferred_time) : "-";
      const date = new Date(p.created_at).toLocaleString("id-ID");
      return [`"${date}"`, `"${parentName}"`, `"${studentName}"`, `"${instrument}"`, `"${schedule}"`, p.amount || 0, p.status].join(",");
    });
    ...
    link.href = url;
    link.setAttribute("download", `Data_Pembayaran_${new Date().toISOString().slice(0, 10)}.csv`);
    ...
    <section style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", gap: "0.75rem", background: "#fff", padding: "1rem 1.25rem", borderRadius: 14, border: "1px solid rgba(39,41,37,0.1)", boxShadow: "0 1px 4px rgba(39,41,37,0.06)" }}>
        ...
        <div style={{ display: "flex", gap: "0.5rem" }}>
          <button onClick={exportToCSV} style={{ padding: "0.5rem 1rem", borderRadius: 10, border: "1px solid rgba(22,163,74,0.3)", background: "rgba(22,163,74,0.07)", color: "#15803d", fontSize: "0.8rem", fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
            Export CSV
          </button>
          <button onClick={fetchAll} style={{ padding: "0.5rem 1rem", borderRadius: 10, border: "1px solid rgba(39,41,37,0.15)", background: "rgba(39,41,37,0.04)", color: "var(--charcoal)", fontSize: "0.8rem", fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
            Refresh
          </button>
        </div>
      </div>
          ...
          <tbody>
            {filteredPayments.length === 0 ? (
              ...
              filteredPayments.map((p) => {
                let q = p.questionnaire_id ? questionnaireById.get(p.questionnaire_id) : null;
                if (!q) q = pickQAsOf(p.user_id, p.created_at);
                const parentName = [q?.father_name, q?.mother_name].filter(Boolean).join(" & ") || "-";
                const preferred = q ? fmtPreferredSchedule(q.preferred_day, q.preferred_time) : "";
                ...
                return (
                  <tr key={p.id} style={{ borderBottom: "1px solid rgba(39,41,37,0.07)" }}>
                    ...
                    <td style={{ padding: "0.7rem 0.9rem", verticalAlign: "top", color: "var(--charcoal)" }}>
                      {parentName}
                      {!p.questionnaire_id && <div style={{ fontSize: "0.68rem", color: "#94A3B8", marginTop: 2 }}>(not linked)</div>}
                    </td>
                    <td style={{ padding: "0.7rem 0.9rem", verticalAlign: "top", fontWeight: 600, color: "var(--charcoal)" }}>{q?.student_full_name || "-"}</td>
                    <td style={{ padding: "0.7rem 0.9rem", verticalAlign: "top", color: "var(--olive)", fontWeight: 600 }}>{q?.instrument || "-"}</td>
                    ...
                    <td style={{ padding: "0.7rem 0.9rem", verticalAlign: "top" }}>
                      {p.proof_url ? (
                        <a href={p.proof_url} target="_blank" rel="noreferrer" style={{ color: "var(--olive)", textDecoration: "underline", fontSize: "0.78rem", fontWeight: 600 }}>Lihat Bukti</a>
                      ) : (
                    ...
                    <td style={{ padding: "0.7rem 0.9rem", verticalAlign: "top", textAlign: "center" }}>
                      <div style={{ display: "flex", justifyContent: "center", gap: "0.4rem" }}>
                        <button
                          ...
                          style={{ padding: "0.3rem 0.7rem", borderRadius: 7, fontSize: "0.75rem", fontWeight: 700, border: "none", cursor: p.status === "verified" ? "not-allowed" : "pointer", background: p.status === "verified" ? "#F1F5F9" : "#16a34a", color: p.status === "verified" ? "#94A3B8" : "#fff", fontFamily: "inherit" }}
                        >
                          Verifikasi
                        </button>
                        <button
                          ...
                          style={{ padding: "0.3rem 0.7rem", borderRadius: 7, fontSize: "0.75rem", fontWeight: 700, border: "none", cursor: p.status !== "pending" ? "not-allowed" : "pointer", background: p.status !== "pending" ? "#F1F5F9" : "#dc2626", color: p.status !== "pending" ? "#94A3B8" : "#fff", fontFamily: "inherit" }}
                        >
                          Tolak
                        </button>
                      </div>
