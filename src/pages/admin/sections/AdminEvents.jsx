...
const EMPTY_EV = {
  title: "", subtitle: "", event_date: "", time_start: "", time_end: "",
  location: "", location_detail: "", city: "Jakarta, Indonesia",
  description: "", tags: "", highlight: false, is_published: true
};
...
export default function AdminEvents({ onCountUpdate }) {
  ...
  const openAddEvent = () => { setEvEditing(null); setEvForm(EMPTY_EV); setEvModal(true); };

  const openEditEvent = (ev) => {
    setEvEditing(ev);
    setEvForm({
      title: ev.title || "", subtitle: ev.subtitle || "",
      event_date: ev.event_date || "", time_start: ev.time_start || "",
      time_end: ev.time_end || "", location: ev.location || "",
  ...
  const deleteEvent = async (ev) => {
    if (!window.confirm(`Hapus event "${ev.title}"? Tidak bisa dibatalkan.`)) return;
  ...
  const toggleEventPublish = async (ev) => {
    ...
    if (!window.confirm(`Apakah Anda yakin ingin ${actionDesc} event "${ev.title}"?`)) return;
    setEventsData(prev => prev.map(item => item.id === ev.id ? { ...item, is_published: !ev.is_published } : item));
                  ...
                  <td className="p-3 align-top">
                    ...
                    {(ev.tags || []).length > 0 && (
                      <div className="flex gap-1 flex-wrap mt-1">
                        {ev.tags.map(t => <span key={t} className="px-1.5 py-0.5 rounded text-[10px] bg-slate-100 text-slate-500">{t}</span>)}
                      </div>
                    )}
                  </td>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={saveEvent} className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                ...
                <input value={evForm.subtitle} onChange={e => setEvForm(s => ({ ...s, subtitle: e.target.value }))} className="w-full px-3 py-2 border-2 border-slate-200 rounded-xl text-sm focus:outline-none focus:border-[#272925] transition" placeholder="An Evening of Young Pianists" />
              </div>
              <div>
